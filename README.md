...
![Main menu image](assets/images/startmenu.png)
...
![Create Account image](assets/images/create_acc.png)
...
![Recovery backup](assets/images/backup.png)
...
![Account Creation complete](assets/images/acc_create_complete.png)
...
![Account Login image](assets/images/acc_login.png)
...
![Account Recovery image](assets/images/recoverymenu.png)
...
![Recovery Questions image](assets/images/recoveryquest.png)
...
![Forgot Recovery image](assets/images/recoveryforgot.png)

## Bank Hub

The Bank Hub of Eternity Holdings is the central function that connects all the banking features together. The banking functions are written in the `run.py` file.

The `bank_hub()` function on line 188 prompts users to enter a list of choices each calling their own functions.

The 'BALANCE' option prints the logged in users balance to the terminal with `login_user_bal()` on line 446.

The 'MORE OPTIONS' sends users to the extended bank hub terminal containing account recovery and account pin change features. This is for extra readability and management of features for the user. The function is `acc_options()` and is found on line 614.

The 'LOG OUT' option, `acc_logout_confirm()` line 766 will prompt the user to confirm they want to log out of their account. If they do the system will return them to the `start_menu()` main menu.

![Bank Hub image](assets/images/bankhub.png)

### Deposit & Withdrawal

The Deposit and Withdrawal system in Eternity Holdings function very similarly. Funds have a maximum of 100,000 Euros worth that can be stored in a single account at given time. The system checks account balance and will deny deposits that exceed the limit or withdrawals of funds below the value of 0.

The `acc_deposit()` on line 268 and `acc_withdrawal()` on line 320 both use `update_acc_bal()` line 372 to take a value and update the logged in user balance. The Money library was imported to currectly format the currency in the database.

![Deposit Image](assets/images/depositimg.png)

### Currency Convertor

Currency Convertor was a important feature to implement. It allows the currency of a account to be switched from the default Euro to 12 different currencies.

In this version of the project the conversion is manually set at a particular rate and stored in the form of a dictionary. In `run.py` file line 40. The currency convertor menu itself, `currency_convert_menu()` explains to the user what to do and prompts the user with 3 options. 

'EXIT' which returns them the `bank_hub()`

'LIST' which prints a list of the available currencies from the `CURRENCY NAMES` dictionary on line 55. The PPrint Python library was imported to have a better visualized dictionary printed to the terminal.

Or to enter the currency the user wishes to convert their account funds to using the short-hand names of the currencies.

![Currency Menu image](assets/images/currencyconvert.png)

When a user changes currency, their funds must change to match the value. The `currency_converter()` function on line 553 handles this.

### Pin Change

Changing Pin was a optional feature implemented for if a User feels their account may be at rosk of being compromised. The `acc_change_pin()` function prompts the user with a confirmation on their pin change and calls `acc_pin_generator()` function in the `utils.py` file line 215.

The new pin gets updated in the database and printed to the terminal for the user to take note of.

![Pin Change image](assets/images/change_pin.png)
...
For Eternity Holdings, some Github commits were larger than anticipated. The reason behind certain commits being very large was due to a major code restructure later in the project timeline. 
